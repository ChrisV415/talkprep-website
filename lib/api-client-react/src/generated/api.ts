...
/**
 * @summary Generate a personalized resignation script (SSE stream)
 */
export const getGenerateResignationScriptUrl = () => {
  return `/api/resignation-script`;
};

export const generateResignationScript = async (
  generateResignationScriptBody: GenerateResignationScriptBody,
  options?: RequestInit,
): Promise<unknown> => {
  return customFetch<unknown>(getGenerateResignationScriptUrl(), {
    ...options,
    method: "POST",
    headers: { "Content-Type": "application/json", ...options?.headers },
    body: JSON.stringify(generateResignationScriptBody),
  });
};

export const getGenerateResignationScriptMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof generateResignationScript>>,
    TError,
    { data: BodyType<GenerateResignationScriptBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  ...
  TContext
> => {
  const mutationKey = ["generateResignationScript"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
  ...
  > = (props) => {
    ...
    return generateResignationScript(data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type GenerateResignationScriptMutationResult = NonNullable<
  Awaited<ReturnType<typeof generateResignationScript>>
>;
export type GenerateResignationScriptMutationBody =
  BodyType<GenerateResignationScriptBody>;
export type GenerateResignationScriptMutationError = ErrorType<unknown>;

/**
 * @summary Generate a personalized resignation script (SSE stream)
 */
export const useGenerateResignationScript = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
  ...
  TContext
> => {
  return useMutation(getGenerateResignationScriptMutationOptions(options));
};
